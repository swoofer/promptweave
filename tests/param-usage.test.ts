...
function makeBehavior(
  overrides: Partial<Behavior> & { name: string },
): Behavior {
  return {
    description: "test",
    category: "mission",
...
function makeRule(
  overrides: Partial<CompositionRule> & { name: string },
): CompositionRule {
  return {
...
describe("findUnusedParams", () => {
  ...
  it("signale un param qu'aucune section ne référence — le bug essaim #79", () => {
    const b = makeBehavior({
      ...
      sections: {
        "030-mission": { prompt: "Six catégories en dur, zéro placeholder." },
      },
    });
  ...
  it("compte {{#if}} et {{#each}} comme des références, pas seulement {{x}}", () => {
    ...
    const b = makeBehavior({
      ...
      sections: {
        "030-mission": {
          prompt:
            "{{#if params.flag}}{{#each params.items}}- {{this}}\n{{/each}}{{/if}}",
        },
      },
    });
  ...
  it("ne signale pas les knobs de phase quand le behavior déclare un bloc phase", () => {
    ...
    const b = makeBehavior({
      ...
      params: {
        ...
        requireFailingTest: { type: "boolean", required: false },
      },
    });
  ...
  it("ne signale pas un param sur lequel une composition fait params_match", () => {
    ...
    const rule = makeRule({
      ...
      when: {
        all: ["coordinator-rules"],
        any: [],
        none: [],
        params_match: { "coordinator-rules": { solo_mode: true } },
      },
    });
  ...
  it("une override_section qui vise un AUTRE behavior n'excuse pas le param", () => {
    ...
    const greeter = makeBehavior({
      name: "greeter",
      params: { who: OPTIONAL },
    });
    const other = makeBehavior({
      name: "other",
      sections: { "030-mission": { prompt: "x" } },
    });
    const rule = makeRule({
      name: "ailleurs",
      actions: {
  ...
  it("un param homonyme consommé par un AUTRE behavior reste signalé", () => {
    ...
    expect(findUnusedParams(map(a, b), [])).toEqual([
      { behavior: "b", param: "focus" },
    ]);
  });
  ...
  it("compte {{../params.x}} — depuis un #each, le frame parent EST le contexte racine", () => {
    ...
    const b = makeBehavior({
      ...
      sections: {
        "030-mission": {
          prompt:
            "{{#each params.items}}- {{this}} {{../params.suffix}}\n{{/each}}",
        },
      },
    });
